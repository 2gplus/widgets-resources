...
export const getProperties = (
    ...
    values.columns.forEach((column, index) => {
        if (column.showContentAs !== "attribute" && !column.sortable) {
...
export const getPreview = (values: DatagridPreviewProps): StructurePreviewProps => {
    ...
    //         {
    //             type: "DropZone",
    //             property: values.bulkAction,
    //             placeholder: "Place buttons here"
    //         } as DropZoneProps
    //     ]
    // } as RowLayoutProps;
    ...
    return {
        type: "Container",
        children: [titleHeader, /*bulkActions,*/ headers, ...Array.from({ length: 5 }).map(() => columns), ...footer]
    };
};

export const check = (values: DatagridPreviewProps): Problem[] => {
    ...
    values.columns.forEach((column: ColumnsPreviewType, index) => {
        ...
        } else if (!column.attribute && values.columnsSortable && column.sortable) {
            errors.push({
                property: `columns/${index + 1}/attribute`,
                message: `An attribute is required when sorting is enabled. Select the 'Attribute' property for column ${column.header}`
            });
        }
