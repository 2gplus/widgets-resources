...
export async function SearchContacts(searchQuery) {
    ...
    return new Promise((resolve, reject) => {
        if (!navigator.contacts) {
            return reject(new Error("SearchContacts action requires cordova-plugin-contacts to be installed in the app"));
        }
        ...
        navigator.contacts.find(searchedFields, contacts => {
            Promise.all(contacts.map(contact => createMxObject(contact)))
                .then(objects => resolve(objects))
                .catch(error => reject(error));
        }, error => reject(errorMessage(error)), { filter: searchQuery, desiredFields: searchedFields });
    });
